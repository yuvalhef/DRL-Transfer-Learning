...
class PolicyNetwork:
    def __init__(self, state_size, action_size, learning_rate, name='acrobot_policy'):
        ...
        with tf.variable_scope(name):
            ...
            self.eligibility = tf.reduce_mean(self.neg_log_prob * self.R_t)

            # Loss & optimizer
            # data_loss = -tf.reduce_sum(self.eligibility)
            reg_losses = slim.losses.get_regularization_losses(scope='policy')
            reg_loss = tf.reduce_sum(reg_losses)
            self.loss = self.eligibility + reg_loss
...
class ValueEstimator:
    def __init__(self, state_size, learning_rate, name='value_network'):
        ...
        with tf.variable_scope(name):
            ...
            self.W2 = tf.get_variable("W2", [20, 1], initializer=tf.contrib.layers.xavier_initializer(seed=0))
...
value = ValueEstimator(state_size, 0.002)

saver = tf.train.Saver()
checkpoint_path = "models/acrobot_expert.ckpt"

# Setup TensorBoard Writer
writer = tf.summary.FileWriter('acrobot_expert/output')
